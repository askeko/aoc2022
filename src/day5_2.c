...
void move_crates(crate_t* crate, FILE* file) {
    ...
    crate_t* temp_ptr3;
    crate_t* temp_ptr4;
    crate_t* transition_ptr;
    crate_t* first_crate = crate;
    for (int i = 0; i < NUM_OF_MOVES; i++) {
        fscanf(file, "move %d from %c to %c\n", &num_to_move, &source, &dest);
        while (crate->next_crate != NULL && crate->next_crate->type != source + 1) {
            crate = crate->next_crate;
        }
        temp_ptr1 = crate;
        if (crate->next_crate != NULL) {
            temp_ptr2 = crate->next_crate;
        }

        for (int j = 1; j < num_to_move; j++) {
            crate = crate->prev_crate;
        }
        temp_ptr3 = crate;
        temp_ptr4 = crate->prev_crate;
        crate = first_crate;

        while (crate->next_crate != NULL && crate->next_crate->type != dest + 1) {
            crate = crate->next_crate;
        }
        if (temp_ptr1->next_crate == NULL) {
            temp_ptr4->next_crate = NULL;
            temp_ptr1->next_crate = crate->next_crate;
            temp_ptr3->prev_crate = crate;
            crate->next_crate = temp_ptr3;
        } else {
            transition_ptr = temp_ptr4;
            temp_ptr2->prev_crate = temp_ptr4;
            transition_ptr->next_crate = temp_ptr2;
            temp_ptr1->next_crate = crate->next_crate;
            temp_ptr3->prev_crate = crate;
            crate->next_crate = temp_ptr3;
        }
